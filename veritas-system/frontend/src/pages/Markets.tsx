...
const SPORT_CATEGORIES = [
  ...
  { id: 'football', label: 'Football', emoji: '⚽', kw: ['football', 'soccer', 'premier league', 'la liga', 'champions league', 'bundesliga', 'serie a', 'afcon', 'world cup', 'copa', 'euros', 'mls', 'epl', 'fa cup', 'barcelona', 'real madrid', 'manchester', 'liverpool', 'arsenal', 'chelsea', 'nigeria', 'ghana', 'senegal', 'inter', 'juventus', 'ac milan', 'psg', 'ligue', 'liga'] },
  ...
  { id: 'boxing', label: 'Boxing/MMA', emoji: '🥊', kw: ['boxing', 'mma', 'ufc', 'fury', 'usyk', 'joshua', 'fight', 'bout', 'knockout', 'bout'] },
  { id: 'formula1', label: 'Formula 1', emoji: '🏎️', kw: ['formula 1', 'f1', 'grand prix', ' gp', 'verstappen', 'hamilton', 'ferrari', 'mercedes', 'red bull'] },
  { id: 'cricket', label: 'Cricket', emoji: '🏏', kw: ['cricket', 'ipl', 't20', 'test match', 'odi', 'bcci', 'ashes'] },
  { id: 'american_football', label: 'NFL', emoji: '🏈', kw: ['nfl', 'american football', 'super bowl', 'touchdown', 'patriots', 'cowboys', 'chiefs'] },
...
function MarketCardEnhanced({ market, onTrade }: { market: AzuroMarket; onTrade: (m: AzuroMarket) => void }) {
  const { t } = useI18n();
    ...
    <div className="card-dark p-5 flex flex-col gap-4 group">
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="relative shrink-0">
          ...
          {shareOpen && (
            <div className="absolute right-0 top-6 z-20 bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-2xl shadow-xl p-2 w-40 animate-slide-down">
              {[
                ...
                <button
                  key={s.label}
                  onClick={(e) => { e.stopPropagation(); s.fn(); }}
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className="bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 border-2 border-emerald-200 dark:border-emerald-500/30 rounded-xl p-3 text-center transition group/btn"
        >
          ...
          <p className="text-xs text-emerald-500 dark:text-emerald-500">{yes ? formatOdds(yes.currentOdds) : '2.00'}x</p>
        </button>
        <button
          onClick={() => onTrade(market)}
        ...
        >
          ...
          <p className="text-xs text-red-500 dark:text-red-500">{no ? formatOdds(no.currentOdds) : '2.00'}x</p>
        </button>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 bg-white dark:bg-white/5 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 w-full sm:w-56 placeholder-gray-400"
          />
          <button
            onClick={() => refetch()}
            className="p-2.5 rounded-xl border border-gray-200 dark:border-white/10 text-gray-500 hover:text-brand-600 dark:hover:text-brand-400 hover:border-brand-300 transition"
            title="Refresh"
          >
            ↻
          </button>
        ...
        {SPORT_CATEGORIES.map((cat) => {
          ...
          return (
            ...
            >
              {cat.emoji} {cat.label}
              {count > 0 && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full ${active ? 'bg-brand-200 dark:bg-brand-700 text-brand-700 dark:text-brand-200' : 'bg-gray-100 dark:bg-white/10 text-gray-500'}`}>
                  {count}
                </span>
              )}
            </button>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 space-y-4">
          ...
          <div className="bg-gray-50 dark:bg-white/5 rounded-2xl p-4 text-left max-w-md mx-auto space-y-2">
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Possible causes:</p>
            <ul className="text-sm text-gray-500 space-y-1 list-none">
              <li>• VITE_AZURO_CHAIN_ID is not set to 137 in Vercel</li>
              <li>• AZURO_CONFIG not set to mainnet</li>
              <li>• Azuro subgraph temporarily unavailable</li>
            </ul>
          </div>
          <button onClick={() => refetch()} className="btn-primary px-6 py-3 rounded-2xl text-sm font-bold">
            Try Again
          </button>
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 space-y-3">
          ...
          <p className="text-gray-500 text-sm">
            {search || category !== 'all'
              ? 'Try clearing your filters to see all markets'
              : 'Markets appear here when available. Make sure VITE_AZURO_CHAIN_ID=137 in Vercel.'}
          </p>
          {(search || category !== 'all') && (
            <button
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((m) => (
            <MarketCardEnhanced key={m.conditionId} market={m} onTrade={setTradingMarket} />
          ))}
        </div>
      )}
